...
//    match trees {
//      case x :: y :: Nil => Fork(x, y, chars(x) + chars(y), weight(x) + weight(y))
//      case _ => combine(trees)
//    }
  ...
  /**
    ...
    */
  def until[A](cond: A => Boolean, iterFunc: A => A)(root: A): A = {
    if (cond(root)) root
    else until(cond, iterFunc)(iterFunc(root))
  }
  ...
  def mergeCodeTables(a: CodeTable, b: CodeTable): CodeTable = {
    def prepend(bit: Bit)(table: CodeTable): CodeTable = table.map(code => (code._1, bit :: code._2))
